<script lang="ts">
	...
	export let tickBarElement: HTMLDivElement | null = null

	/** Obtains a bound DOM reference to the slider's thumb element. */
	export let thumbElement: HTMLDivElement | null = null

	/** Obtains a bound DOM reference to the slider's outer rail element. */
	export let railElement: HTMLDivElement | null = null

	/** Obtains a bound DOM reference to the slider's track (fill) element. */
	export let trackElement: HTMLDivElement | null = null
	...
	const valueToPercentage = (v: number) => ((v - min) / (max - min)) * 100
...
<style lang="postcss">
	.slider {
		@apply relative
		 bg-black w-6 flex flex-col;
		/* box-shadow: inset 0.625rem 0 white, inset -0.625rem 0 white; */
	}
	...
	.slider-thumb {
		@apply absolute w-full h-2 bg-yellow-500;
